...
Often when working with side effects, a pattern appears where exceptions need to be converted into types. That means that bits of
code that can throw an `Exception` will need to be converted into an `Either[L, R]`.
...
This is nice, but we could take it a bit further. Thinking in a _pythonic_ frame of mind, we could just use regular functions and
add a decorator that will convert the function result to `Either`.
...
We are going to use this in the next bit.

### exchange_customer_information

To avoid wrapping the exception with another exception that doesn't add much, we are going to use `ClientError` as a
possible _left_ value.
...
{: .box-note}
The `curry` decorator creates a function that can be called with fewer arguments and returns another function.
...
This implementation can reuse what we created for the previous function but needs to address two more error scenarios
in case the subscription is missing or expired.
...
The last bit is to store the customer information in the database:
...
That breaks a tad the _chaining_ we were hoping to implement.

I could change the order a bit and _persist the information_ before the code can confirm a valid subscription exists.
...
def request_handler(request: Request):
  return extract_token_from_url(request.url)   \
    ...
    .either(
      redirect_to('Landing', error_to_dict(error)),
      redirect_to('Dashboard', {"expiration_date": subscription.expiration_date})
    )
```
...
I see the following benefits of using the `Either` type:

* Code is very declarative.
* Flow is clear, easy to follow, and improved for readability.
* Errors are explicit, forcing the next person to handle them.
* All errors are handled matching the [error analysis](#error-analysis) done before.
* Using exceptions as errors when convenient; no need to wrap it in another exception.
* Logging is handled in a central place without extra effort.
* Changing where the errors are captured is a simple change.
